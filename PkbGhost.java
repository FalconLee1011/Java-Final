...
public class PkbGhost extends Thread {
  ...
  public boolean pursue(PkbHuman player) {
    ...
    } else if (this.isRangedTrigger) {
      pursueIsRangedTrigger(player);
    } else {
      pursueIsNotRangedTrigger(player);
    }
  ...
  private void pursueIsNotRangedTrigger(PkbHuman player) {
    if (isMoving) {
      ...
      if ((this.x + 120 >= this.rndx || this.x - 120 <= this.rndx)
          && (this.y + 120 >= this.rndy || this.y - 120 <= this.rndy)) {
        this.isMoving = false;
      }
  ...
  private boolean hasCapturedPlayer(PkbHuman player) {
    ...
    Rectangle playerPoly = new Rectangle(player.x - ((player.width - tolerance) / 2),
        player.y - ((player.height - tolerance) / 2), player.width - tolerance, player.height - tolerance);
    if (ghostPoly.intersects(playerPoly)) {
      return true;
    }
    return false;
  ...
  private boolean hasBumpedIntoRock(PkbHuman player) {
    ...
    for (PkbFlyingRock fock : g.flyingRocks) {
      Rectangle fockPoly = new Rectangle(fock.x - (fock.width / 2), fock.y - (fock.height / 2), fock.width,
          fock.height);
      if (ghostPoly.intersects(fockPoly) && fock.valid) {
        return true;
      }
    }
    return false;
